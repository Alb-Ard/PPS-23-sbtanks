 ...
 */
case class PositionProvider(width: Double, height: Double, strategy: PositionStrategy[(Double, Double), (Double, Double)])(collisionMask: Seq[CollisionLayer], collisionSizeX: Double, collisionSizeY: Double):
    /**
    ...
    private def isPositionAvailable(x: Double, y: Double) =
        PhysicsWorld.getBoxOverlaps(AABB(x, y, collisionSizeX, collisionSizeY), collisionMask, Seq.empty).isEmpty
