...
abstract class LevelBaseDeathController[VSlotKey](using context: EntityRepositoryContext[Stage, VSlotKey, Pane])(repository: EntityMvRepositoryContainer[AnyRef, Node], levelSequencer: LevelSequencer[AnyRef, Node], uiSlotKey: VSlotKey)
    ...
    repository.modelViewRemoved += { (m, _) =>
        m match
            case p: LevelObstacle with DamageableBehaviour if baseObstacle.contains(p) => baseObstacle = Option.empty
            case _ => ()
    }
    repository.entitiesOfModelType[LevelObstacle with DamageableBehaviour].headOption.foreach(onModelCreated)
    ...
    private def onModelCreated(m: AnyRef) = m match
        case p: LevelObstacle with DamageableBehaviour if(p.obstacleType == playerBase) =>
            baseObstacle.foreach(p => p.destroyed -= gameover)
            baseObstacle = Option(p)
